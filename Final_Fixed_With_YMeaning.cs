...
namespace WpfChartExcelExport
{
    public static class LiveCharts2ScatterWorksheetExporter
    {
            ...
            IComparer<double>? xComparer = null)
        {
            ...
            int metaRow1 = startRow;
            int metaRow2 = startRow + 1;

            // 👇 NEW: X and Y meaning rows
            worksheet.Cells[metaRow1, startColumn].SetValue("X Meaning");
            worksheet.Cells[metaRow1, startColumn + 1].SetValue(xAxisTitle ?? "X");

            worksheet.Cells[metaRow2, startColumn].SetValue("Y Meaning");
            worksheet.Cells[metaRow2, startColumn + 1].SetValue(yAxisTitle ?? "Y");

            int headerRow = startRow + 3;
            ...
            string? yAxisTitle)
        {
            ...
            for (int i = 0; i < mapped.Count; i++)
            {
                ...
                group.Series.Add(xVals, yVals, new TextTitle(mapped[i].Name));
            }
