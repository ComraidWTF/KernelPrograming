    ...
    public static class OpenXmlLiveChartsExporter
    {
            ...
            IComparer<double>? xComparer = null)
        {
            ...
            MappedSeries? MapSeries(ISeries s)
            {
                ...
                return new MappedSeries(name, TryGetSeriesHexColor(s), points);
            }
        }
            ...
            for (int i = 0; i < mapped.Count; i++)
            {
                ...
                var ser = new C.ScatterChartSeries(
                    ...
                    new C.Marker(
                        ...
                        new C.Size { Val = 7 },
                        CreateMarkerShapeProperties(mapped[i].HexColor)),
        ...
        {
            string rgb = NormalizeHex(hexColor);
        ...
        private static A.ShapeProperties CreateMarkerShapeProperties(string hexColor)
        {
            string rgb = NormalizeHex(hexColor);
            return new A.ShapeProperties(
                new A.SolidFill(new A.RgbColorModelHex { Val = rgb }),
        ...
        private static string TryGetSeriesHexColor(ISeries series)
        {
            string[] candidateProps = new[] { "Stroke", "Fill", "GeometryStroke" };
            foreach (var propName in candidateProps)
            {
                var prop = series.GetType().GetProperty(propName);
                if (prop == null) continue;
                var paint = prop.GetValue(series);
                var hex = TryGetHexFromPaint(paint);
                if (!string.IsNullOrWhiteSpace(hex)) return hex;
            }

            int stableIndex = Math.Abs((series.Name ?? "Series").GetHashCode());
            return DefaultPaletteColor(stableIndex);
        }

        private static string TryGetHexFromPaint(object? paint)
        {
            if (paint == null) return null;

            var colorProp = paint.GetType().GetProperty("Color");
            if (colorProp == null) return null;

            var color = colorProp.GetValue(paint);
            if (color == null) return null;

            var redProp = color.GetType().GetProperty("Red");
            var greenProp = color.GetType().GetProperty("Green");
            var blueProp = color.GetType().GetProperty("Blue");
            if (redProp == null || greenProp == null || blueProp == null) return null;

            try
            {
                byte r = Convert.ToByte(redProp.GetValue(color), CultureInfo.InvariantCulture);
                byte g = Convert.ToByte(greenProp.GetValue(color), CultureInfo.InvariantCulture);
                byte b = Convert.ToByte(blueProp.GetValue(color), CultureInfo.InvariantCulture);
        ...
        {
            if (string.IsNullOrWhiteSpace(hexColor)) return "4472C4";
            string clean = hexColor.Trim().TrimStart('#');
            return clean.Length == 6 ? clean.ToUpperInvariant() : "4472C4";
        }
        ...
        private sealed class MappedSeries
        {
            ...
            {
                Name = name;
                HexColor = string.IsNullOrWhiteSpace(hexColor) ? DefaultPaletteColor(0) : hexColor;
                Points = points;
            }
