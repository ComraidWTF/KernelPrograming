...
public sealed class LiveChartsExcelExporter
{
    ...
    private const uint FirstBlockRow = 5U;
    private const uint TitleStyleIndex = 1U;
    private const string TitleMergeRef = "A1:J3";
    ...
    public void Save()
    {
        ...
        {
            var prepared = _sheets[name].Select(Prepare).OfType<PreparedChart>().ToList();
    ...
    private static void WriteSheet(WorksheetPart wsPart, DrawingsPart drPart, string name, List<PreparedChart> prepared)
    {
        ...
        var titleRow = new Spreadsheet.Row { RowIndex = WorksheetHeaderRow, Height = 24D, CustomHeight = true };
        titleRow.Append(TextCell(CellRef(XValueCol, WorksheetHeaderRow), name, TitleStyleIndex));
        sheetData.Append(titleRow);
        sheetData.Append(new Spreadsheet.Row { RowIndex = 2U, Height = 24D, CustomHeight = true });
        sheetData.Append(new Spreadsheet.Row { RowIndex = 3U, Height = 24D, CustomHeight = true });

        uint cursor = FirstBlockRow;
        ...
        wsPart.Worksheet = new Spreadsheet.Worksheet(
            ...
            sheetData,
            new Spreadsheet.MergeCells(
                new Spreadsheet.MergeCell { Reference = TitleMergeRef })
            { Count = 1U },
    ...
    private static PreparedChart? Prepare(ChartExportSpec spec)
    {
        ...
        var xSel = spec.XSelector ?? throw new ArgumentException("XSelector is required on every ChartExportSpec.");
        var ySel = spec.YSelector ?? throw new ArgumentException("YSelector is required on every ChartExportSpec.");

        var comparer = spec.XComparer ?? Comparer<double>.Default;
        var mapped = spec.Series
            .Select((s, i) => MapSeries(s, i, xSel, ySel))
            .OfType<MappedSeries>().ToList();
    ...
    private static Spreadsheet.Cell TextCell(string cellRef, string? text, uint? styleIndex = null)
    {
    ...
    private static Spreadsheet.Stylesheet BuildStylesheet() => new(
        new Spreadsheet.Fonts(
            ...
            new Spreadsheet.Font(
                ...
                new Spreadsheet.FontSize { Val = 18D },
                new Spreadsheet.FontName { Val = "Calibri" })
        ) { Count = 2U },
        ...
        new Spreadsheet.Borders(
            new Spreadsheet.Border(
                new Spreadsheet.LeftBorder(), new Spreadsheet.RightBorder(),
                new Spreadsheet.TopBorder(), new Spreadsheet.BottomBorder(),
                new Spreadsheet.DiagonalBorder())
        ) { Count = 1U },
        new Spreadsheet.CellStyleFormats(
            new Spreadsheet.CellFormat { NumberFormatId = 0U, FontId = 0U, FillId = 0U, BorderId = 0U }
        ) { Count = 1U },
        new Spreadsheet.CellFormats(
            new Spreadsheet.CellFormat { NumberFormatId = 0U, FontId = 0U, FillId = 0U, BorderId = 0U, FormatId = 0U },
            new Spreadsheet.CellFormat(
                new Spreadsheet.Alignment
                {
            ...
            {
                NumberFormatId = 0U, FontId = 1U, FillId = 0U, BorderId = 0U, FormatId = 0U,
                ApplyFont = true, ApplyAlignment = true
            }
        ) { Count = 2U });
