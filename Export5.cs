    ...
    public static class LiveCharts2WorksheetExporter
    {
            ...
            where TX : notnull
        {
            ...
            for (int r = 0; r < xValues.Count; r++)
            {
                ...
                {
                    int valueColumn = startColumn + 1 + s;

                    if (lookups[s].TryGetValue(x, out var point) && point.Y.HasValue)
                    {
                        worksheet.Cells[dataRow + r, valueColumn].SetValue(point.Y.Value);
                    }
                    else
                    {
                        worksheet.Cells[dataRow + r, valueColumn].SetValueAsFormula("=NA()");
                    }
                }
            }
        }
