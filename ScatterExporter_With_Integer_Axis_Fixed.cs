...
namespace WpfChartExcelExport
{
    public static class LiveCharts2ScatterWorksheetExporter
    {
            ...
            IComparer<double>? xComparer = null)
        {
            ...
            if (xValues.Count == 0) throw new InvalidOperationException("No data points found.");
            ...
            bool showDataLabels)
        {
            ...
            TryConfigureWholeNumberAxis(chart, isX: true, majorUnit: 1, minimum: 1, maximum: null);
            TryConfigureWholeNumberAxis(chart, isX: false, majorUnit: 1, minimum: 0, maximum: null);
            ...
            for (int i = 0; i < mapped.Count; i++)
            {
                ...
                if (showDataLabels)
                {
                    TryConfigureDataLabels(added, "Above", showValue: true);
                }
            }
        ...
        private static void TryConfigureDataLabels(object series, string position, bool showValue)
        {
            try
            {
                ...
                SetEnum(labels.GetType(), labels, "Position", position);
                SetEnum(labels.GetType(), labels, "LabelPosition", position);
        ...
        private static void SetEnum(Type type, object target, string propertyName, string value)
        {
            var p = type.GetProperty(propertyName);
            ...
            try
            {
                var enumValue = Enum.Parse(p.PropertyType, value, true);
        ...
        private static void SetDouble(Type type, object target, string propertyName, double value)
        {
            var p = type.GetProperty(propertyName);
        ...
        private static void SetString(Type type, object target, string propertyName, string value)
        {
            var p = type.GetProperty(propertyName);
            if (p != null && p.CanWrite)
            {
            ...
            string? fontHex = null)
        {
            ...
            {
                style.Fill = PatternFill.CreateSolidFill(HexToColor(fillHex));
            }

            if (!string.IsNullOrWhiteSpace(fontHex))
            {
                style.ForeColor = HexToColor(fontHex);
            }
